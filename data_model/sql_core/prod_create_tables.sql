...
CREATE TABLE m_descriptor (
  m_descriptor_id serial8,
	...
	parent_id int8,
	parent_path ltree,
	material_refname_description_in varchar COLLATE "pg_catalog"."default",
  material_refname_type_in varchar(255) COLLATE "pg_catalog"."default",
...
ALTER TABLE m_descriptor
	...
	ADD CONSTRAINT "un_m_descriptor" UNIQUE (parent_id, material_refname_description_in, material_refname_type_in, m_descriptor_def_id);
...
ALTER TABLE tag_x 
	...
	ADD CONSTRAINT "un_tag_x" UNIQUE (ref_uuid, tag_id);
...
ALTER TABLE tag_x 
	ADD CONSTRAINT fk_tag_x_tag_1 FOREIGN KEY (tag_id) REFERENCES tag (tag_id);
...
CREATE OR REPLACE VIEW vw_actor AS
	SELECT act.actor_uuid as actor_uuid, act.actor_id, org.organization_id, per.person_id, st.systemtool_id, 
		act.description as actor_description, sts.description as actor_status, nt.notetext as actor_notetext, doc.edocument as actor_document, doc.edoc_type as actor_doc_type,
		org.full_name as org_full_name, org.short_name as org_short_name, per.lastname as per_lastname, per.firstname as per_firstname, 
		case when per.person_id is not null then cast(concat(per.lastname,', ',per.firstname) as varchar) end as person_lastfirst, porg.full_name as person_org,
		st.systemtool_name, st.description as systemtool_description, stt.description as systemtool_type, st.vendor as systemtool_vendor, 
		st.model as systemtool_model, st.serial as systemtool_serial, st.ver as systemtool_version, storg.full_name as systemtool_org
	from actor act 
...
FROM systemtool stl 
JOIN 
	(SELECT st.systemtool_name, st.systemtool_type_id, st.organization_id, MAX ( st.ver ) AS ver 
	FROM systemtool st 
	WHERE st.systemtool_name IS NOT NULL AND st.ver IS NOT NULL 
	GROUP BY st.systemtool_name, st.systemtool_type_id, st.organization_id ) mrs 
ON stl.systemtool_name = mrs.systemtool_name AND stl.systemtool_type_id = mrs.systemtool_type_id AND stl.organization_id = mrs.organization_id AND stl.ver = mrs.ver;
...
SELECT act.*  
FROM vw_latest_systemtool vst 
JOIN actor act
on vst.systemtool_id = act.systemtool_id;
...
/*
	...
	md.parent_id,
	md.parent_path,
	md.material_refname_description_in,
	md.material_refname_type_in,
	md.m_descriptor_def_id,
	md.create_date,
	md.num_valarray_out,
...
FROM
	m_descriptor md
	left JOIN m_descriptor_def mdd ON md.m_descriptor_def_id = mdd.m_descriptor_def_id
	left JOIN vw_actor dact ON md.actor_id = dact.actor_id
	left join vw_actor dfact on mdd.actor_id = dfact.actor_id
