...
def get_all_related_fields_backward(model):
    def _get_all_fields_rec(
        model, all_related_fields, parent_model=None, parent_field_name=None
    ):
        ...
        for field_obj in model_fields:
            field_details = FrozenDict(**{"model": model, "field_name": field_obj.name})
            parent_field_details = FrozenDict(
                **{"model": parent_model, "field_name": parent_field_name}
            )
    ...
    all_related_fields = {FrozenDict(**{"model": None, "field_name": None})}
...
def get_all_related_fields(top_level_model):
    ...
    def get_all_related_fields_rec(model, parent_model=None):
        ...
        for field_obj in model_fields:
            ...
            field_details = FrozenDict(**{"model": model, "field_name": field_name})
            field_rel_class = getattr(field_obj, "rel_class", None)
            if field_rel_class:
                field_rel_class_name = field_rel_class.__name__
                if field_rel_class_name != "ManyToManyRel":
