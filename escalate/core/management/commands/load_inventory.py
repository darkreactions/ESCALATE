...
inventory_to_add = [dict(zip(inventory_relevant_fields, data)) for data in inventory_data]
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for fields_bunch in inventory_to_add:
            inventory_instance, created = Inventory.objects.get_or_create(**fields_bunch)
            if created:
                self.stdout.write(self.style.SUCCESS(f'Created inventory {inventory_instance}'))
            else:
                self.stdout.write(self.style.NOTICE(f'Did NOT create inventory {inventory_instance}, already exists'))
