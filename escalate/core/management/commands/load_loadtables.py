...
class Command(BaseCommand):
    ...
    def _create_wf1(self):
        ...
        lab = Actor.objects.get(description='Haverford College', 
                                person__isnull=True, systemtool__isnull=True)

        # Create the experiment
        exp_template = ExperimentTemplate(description='Workflow 1', 
                                          ref_uid='workflow_1',
                                          lab=lab,)
        exp_template.save()
    ...
    def _create_wf3(self):
        ...
        # Create the experiment
        exp_template = ExperimentTemplate(description='Workflow 3', 
                                          ref_uid='workflow_3',
                                          lab=lab,)
        exp_template.save()

        reagents = {'Reagent 2':['organic', 'solvent'], 
                    'Reagent 7':['acid'], 
                    'Reagent 3':['inorganic', 'organic', 'solvent'], 
                    'Reagent 1':['solvent'], 
                    'Reagent 9': ['solvent']} #should we classify antisolvent as a solvent? and where do we specify the identity of it?
        ...
        default_dead_volume, created = DefaultValues.objects.get_or_create(**{'description':'WF3 dead volume', 
                                                              'nominal_value': dead_vol_val,
                                                              'actual_value': dead_vol_val
                                                              })
        default_amount, created = DefaultValues.objects.get_or_create(**{'description':'Zero gm', 
        ...
        action_sequences = {
            ...
            'Stir Rate (rpm)': ActionSequence.objects.create(description='Stir Rate (rpm)'),
            'Reaction time (s)': ActionSequence.objects.create(description='Reaction time (s)'),
        } #where do we specify the actual parameter values, like the temperature?
        ...
        for row in range(rows):
            if (row+1)%2!=0:
                ...
                for col in column_order[4:]:
                    ...
                    well_list.append('{}{}'.format(col, row+1))
        plate = Vessel.objects.get_or_create(description='WF3 Plate well')
        # Dictionary of plate wells so that we don't keep accessing the database
        # multiple times
        a_wells = {}
        b_wells = {}
        ...
        action_parameter_def = {
            ...
            'stir': ('temperature', 'duration', 'speed'),
            'heat': ('temperature', 'duration'),
            'cool': ('temperature', 'duration'),
            'store': ('temperature', 'duration')
        }
        # Action defs it is assumed that action defs are already inserted 
        actions = [ # List of tuples (Description, Action def description, source_bommaterial, destination_bommaterial)
            # Preheat plate to 85 degrees C
            ('Preheat Temperature (C)', 'bring_to_temperature', (None, None), ('vessel', 'WF3 Plate well'), 'Preheat Temperature (C)'),
            # Prepare stock A
            ('Add Solvent to Stock A', 'dispense', (None, 'Solvent'), (None, 'Stock A Vial'), 'Prepare stock A'),
            ('Add Organic to Stock A', 'dispense', (None, 'Organic'), (None, 'Stock A Vial'), 'Prepare stock A'),
            ('Add Inorganic to Stock A', 'dispense', (None, 'Inorganic'), (None, 'Stock A Vial'), 'Prepare stock A'),
            ...
            # Dispense Acid Vol 1
            ('Dispense Acid Vol 1', 'dispense', (None, 'Solvent'), ('vessel', a_wells), 'Dispense Acid Volume 1'),
            # Heat stir 1
            ('Heat stir 1', 'heat_stir', (None, None), ('vessel', 'WF3 Plate well'), 'Heat stir 1'),
            ('Mixing time1 (s)', 'stir', (None, None), ('vessel', 'WF3 Plate well'), 'Mixing time1 (s)'),
            # Dispense Acid Vol 2
            ('Dispense Acid Vol 2', 'dispense', (None, 'Solvent'), ('vessel', a_wells), 'Dispense Acid Volume 2'),
            # Heat stir 2
            ('Heat stir 2', 'heat_stir', (None, None), ('vessel', 'WF3 Plate well'), 'Heat stir 2'),
            ('Mixing time2 (s)', 'stir', (None, None), ('vessel', 'WF3 Plate well'), 'Mixing time2 (s)'),
            # Cool to 26 C
            ('Cool', 'cool', (None, None), ('vessel', 'WF3 Plate well'), 'Cool'),
            ('Temperature (C)', 'cool', (None, None), ('vessel', 'WF3 Plate well'), 'Temperature (C)'),
            ('Stir Rate (rpm)', 'stir', (None, None), ('vessel', 'WF3 Plate well'), 'Stir Rate (rpm)'),
            ('Reaction time (s)', 'stir', (None, None), ('vessel', 'WF3 Plate well'), 'Reaction time (s)')
            # Dispense antisolvent
            ('Dispense Antisolvent', 'dispense', (None, 'Solvent'), ('vessel', b_wells), 'Dispense Antisolvent')
            # Store at room temp, no heating or stirring
            ('Store', 'store', (None, None), ('vessel', 'WF3 Plate well'), 'store'),
            ('Temperature (C)', 'store', (None, None), ('vessel', 'WF3 Plate well'), 'Temperature (C)'),
            ('Reaction time (s)', 'store', (None, None), ('vessel', 'WF3 Plate well'), 'Reaction time (s)'),
        ]
        ...
        with open(LOAD_CHEM_INVENTORY, newline='') as f:
            ...
            for row in reader:
                ...
                if created:
                    new_mat_counter += 1
            self.stdout.write(self.style.SUCCESS(
                f'Added {new_mat_counter} new materials'))
    ...
    def _load_vessels(self):
        ...
        with open(OLD_DEV_SCHEMA_MATERIALS, newline='') as f:
            ...
            for row in reader:
                ...
                plate_instance, created = Vessel.objects.get_or_create(description=plate_name)
                if well_number is not None:
                    well_instance, created = Vessel.objects.get_or_create(description=well_number, 
                                                                          parent=plate_instance)
                    if created:
                        new_plates += 1
                """
                fields = {
        ...
        with open(PARAMETER_DEF, newline='') as f:
            ...
            for row in reader:
                ...
                if created:
                    new_parameter_def += 1
            self.stdout.write(self.style.SUCCESS(
                f'Added {new_parameter_def} new parameter def'))
        ...
        with open(ACTION_DEF, newline='') as f:
            ...
            for row in reader:
                ...
                action_def_instance.parameter_def.add(
                    *[ParameterDef.objects.get(description=x) for x in parameter_def_descriptions])
            self.stdout.write(self.style.SUCCESS(
    ...
    def _load_calculation_def(self):
        ...
        with open(CALCULATION_DEF, newline='') as f:
            ...
            for row in reader:
                description = clean_string(
                    row[column_names_to_index['description']])
                short_name = clean_string(
                    row[column_names_to_index['short_name']])
    ...
    def _load_mixture(self):
        ...
        with open(MIXTURE, newline='') as f:
            ...
            for row in reader:
                ...
                addressable = clean_string(
                    row[column_names_to_index['addressable']])
                fields = {
    ...
    def _load_base_bom_material(self):
        ...
        with open(BASE_BOM_MATERIAL, newline='') as f:
            ...
            for row in reader:
                ...
                bom_material_bom = BillOfMaterials.objects.get(
                    description=y) if not string_is_null(y := bom_material_bom_description) else None
    ...
    def _load_action(self):
        ...
        with open(ACTION, newline='') as f:
            ...
            for row in reader:
                ...
                repeating = clean_string(
                    row[column_names_to_index['repeating']])
                ...
                parameter_def_description = [x.strip() for x in y.split(',')] if not string_is_null(
                    y := row[column_names_to_index['parameter_def_description']]) else []
                action_instance.parameter_def.add(
                    *[ParameterDef.objects.get(description=d) for d in parameter_def_description])

            self.stdout.write(self.style.SUCCESS(
                f'Added {new_action} new action'))
