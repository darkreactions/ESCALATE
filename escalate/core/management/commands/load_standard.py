...
for model_name, field_names in model_field_names.items():
    model_instances = [
        dict(zip(field_names, data)) for data in model_field_data[model_name]
    ]
        ...
        for model_name_raw, array_of_fields in standard.items():
            ...
            for fields_bunch in array_of_fields:
                ...
                if created:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"Created {model_name} {model_instance}")
                    )
                else:
                    self.stdout.write(
                        self.style.NOTICE(
                            f"Did NOT create {model_name} {model_instance}, already exists"
                        )
                    )
