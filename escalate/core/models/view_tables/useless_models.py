...
class BomMaterialIndex(DateColumns):
    ...
    uuid = RetUUIDField(
        primary_key=True, default=uuid.uuid4, db_column="bom_material_index_uuid"
    )
    ...
    bom_composite_material = models.ForeignKey(
        "BomCompositeMaterial",
        on_delete=models.DO_NOTHING,
        blank=True,
        null=True,
        db_column="bom_material_composite_uuid",
...
class CalculationParameterDefAssign(DateColumns):
    uuid = RetUUIDField(
        primary_key=True,
        default=uuid.uuid4,
        db_column="calculation_parameter_def_x_uuid",
    )
    parameter_def = models.ForeignKey(
        "ParameterDef",
        on_delete=models.DO_NOTHING,
...
class ParameterX(DateColumns):
    uuid = RetUUIDField(
        primary_key=True, default=uuid.uuid4, db_column="parameter_x_uuid"
    )
...
class MaterialTypeAssign(DateColumns):
    uuid = RetUUIDField(primary_key=True, db_column="material_type_x_uuid")
...
class BomCompositeMaterial(DateColumns, StatusColumn, ActorColumn):
    uuid = RetUUIDField(
        primary_key=True, default=uuid.uuid4, db_column="bom_material_composite_uuid"
    )
    description = models.CharField(max_length=255, blank=True, null=True)
...
class Condition(DateColumns, StatusColumn, ActorColumn):
    ...
    uuid = RetUUIDField(
        primary_key=True, default=uuid.uuid4, db_column="condition_uuid"
    )
    """
...
class ConditionDef(DateColumns, StatusColumn, ActorColumn, DescriptionColumn):
    ...
    calculation_def = (
        models.ManyToManyField("CalculationDef", blank=True),
...
class ConditionPath(DateColumns):
    ...
    internal_slug = SlugField(
        populate_from=["condition__internal_slug", "workflow_step__internal_slug"],
        overwrite=True,
        max_length=255,
    )
...
class ParameterEditView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if "pk" in kwargs:
            ...
            q1 = (
                ActionParameter.objects.filter(
                    workflow__experiment_workflow_workflow__experiment=kwargs["pk"]
                )
                .only("uuid")
                .annotate(object_description=F("action_description"))
                .annotate(object_uuid=F("uuid"))
                .annotate(parameter_value=F("parameter_val_nominal"))
                .annotate(experiment_uuid=F(f"{related_exp}__uuid"))
                .annotate(experiment_description=F(f"{related_exp}__description"))
                .annotate(workflow_seq=F(f"{related_exp_wf}__experiment_workflow_seq"))
                .filter(workflow_action_set__isnull=True)
                .prefetch_related(f"{related_exp}")
            )

            q2 = (
                WorkflowActionSet.objects.filter(
                ...
                .annotate(object_uuid=F("uuid"))
                .annotate(parameter_def_description=F("parameter_def__description"))
                .annotate(parameter_uuid=Value(None, RetUUIDField()))
                .annotate(parameter_value=F("parameter_val_nominal"))
                .annotate(experiment_uuid=F(f"{related_exp}__uuid"))
                .annotate(experiment_description=F(f"{related_exp}__description"))
                .annotate(workflow_seq=F(f"{related_exp_wf}__experiment_workflow_seq"))
            ...
            q3 = (
                ...
                .annotate(
                    parameter_uuid=F(
                        "calculation__calculation_def__parameter_def__uuid"
                    )
                )
                .annotate(
                    parameter_value=F(
            ...
            initial_q2 = [
                {"value": param} for row in q2 for param in row.parameter_value
            ]
            initial_q3 = [{"value": row.parameter_value} for row in q3]

            q1_details = [
                f"{row.object_description} : {row.parameter_def_description}"
                for row in q1
            ]
            q2_details = [
                f"{row.object_description} : {row.parameter_def_description}"
            ...
            context["q1_formset"] = self.ParameterFormSet(
                initial=initial_q1, prefix="q1"
            )
            context["q2_formset"] = self.ParameterFormSet(
                initial=initial_q2, prefix="q2"
            )
            context["q3_formset"] = self.ParameterFormSet(
                initial=initial_q3, prefix="q3"
            )
...
class MaterialEditView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if "pk" in kwargs:
            experiment = Experiment.objects.get(pk=kwargs["pk"])
            q1 = (
                BomMaterial.objects.filter(bom__experiment=experiment)
                .only("uuid")
                .annotate(object_description=F("description"))
                .annotate(object_uuid=F("uuid"))
                .annotate(experiment_uuid=F(f"{related_exp}__uuid"))
            ...
            context["q1_formset"] = self.MaterialFormSet(
                initial=initial_q1, prefix="q1", form_kwargs=form_kwargs
            )
            context["q1_details"] = q1_details
...
class WorkflowActionSet(DateColumns, StatusColumn, ActorColumn, DescriptionColumn):
    uuid = RetUUIDField(
        primary_key=True, default=uuid.uuid4, db_column="workflow_action_set_uuid"
    )
    ...
    parameter_val_nominal = CustomArrayField(
        ValField(), blank=True, null=True, db_column="parameter_val"
    )
    parameter_val_actual = CustomArrayField(
        ValField(), blank=True, null=True, db_column="parameter_val_actual"
    )
    ...
    source_material = ArrayField(
        RetUUIDField(blank=True, null=True), db_column="source_uuid"
    )
    destination_material = ArrayField(
        RetUUIDField(blank=True, null=True), db_column="destination_uuid"
    )


class WorkflowStep(DateColumns, StatusColumn):
    uuid = RetUUIDField(
        primary_key=True, default=uuid.uuid4, db_column="workflow_step_uuid"
    )
    action_sequence = models.ForeignKey(
        "ActionSequence",
        models.CASCADE,
    ...
    action = models.ForeignKey(
        ...
        related_name="workflow_step_action",
    )
    ...
    internal_slug = SlugField(
        populate_from=["workflow__internal_slug", "action__internal_slug"],
        overwrite=True,
        max_length=255,
    )
...
class WorkflowObject(DateColumns, StatusColumn):
    uuid = RetUUIDField(
        primary_key=True, default=uuid.uuid4, db_column="workflow_object_uuid"
    )
    action_sequence = models.ForeignKey(
        "ActionSequence",
        models.CASCADE,
        ...
        null=True,
        db_column="workflow_uuid",
        related_name="workflow_object_workflow",
    )
    action = models.ForeignKey(
        "Action",
        models.CASCADE,
    ...
    workflow_action_set = models.ForeignKey(
        "WorkflowActionSet",
        models.CASCADE,
        blank=True,
        null=True,
