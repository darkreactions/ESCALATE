...
def liquid_solid_extraction(data, q3, experiment_copy_uuid, exp_name, exp_template):
    ...
    xls_edoc = Edocument.objects.get(
        ref_edocument_uuid=exp_template.uuid, title="XLS file"
    )

    hcl_vols, h2o_vols = hcl_mix(
        data["stock_concentration"],
        data["total_vol"],
        np.fromstring(data["hcl_concentrations"].strip("]["), sep=","),
    )
    h2o_dispense_action_set = WorkflowActionSet.objects.get(
        **{f"{related_exp}": experiment_copy_uuid, "description__contains": "H2O"}
    )
    hcl_dispense_action_set = WorkflowActionSet.objects.get(
        **{f"{related_exp}": experiment_copy_uuid, "description__contains": "HCl"}
    )
    update_dispense_action_set(h2o_dispense_action_set, h2o_vols)
    update_dispense_action_set(hcl_dispense_action_set, hcl_vols)
    new_lsr_pk, lsr_msg = update_lsr_edoc(
        lsr_edoc,
