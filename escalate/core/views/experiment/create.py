...
from core.forms.custom_types import (SingleValForm, InventoryMaterialForm, NominalActualForm, 
                                     ...
                                     ReagentSelectionForm, MaterialTypeSelectionForm,
...
class CreateExperimentView(TemplateView):
    ...
    def get_material_forms(self, exp_uuid: str, context: dict[str, Any]) -> dict[str, Any]:
        ...
        initial_q1: list = [{'value': row.inventory_material, 'uuid': json.dumps([f'{row.object_description}'])} for row in q1]
        q1_details: list = [f'{row.object_description}' for row in q1]
        
        form_kwargs: dict[str, Any] = {'org_uuid':self.request.session['current_org_id']}
    ...
    def get_colors(self, number_of_colors: int, 
                   colors: list[str] = ['lightblue', 'teal', 'powderblue', 
                                        'skyblue', 'pastelblue', 'verdigris', 
                                        'steelblue', 'cornflowerblue']) -> list[str]:
      factor = int(number_of_colors/len(colors))    
      remainder = number_of_colors % len(colors)
      total_colors = colors*factor + colors[:remainder]         
      return total_colors
    
    def get_reagent_forms(self, exp_template: ExperimentTemplate, context: dict[str, Any]):
        if 'current_org_id' in self.request.session:
            org_id = self.request.session['current_org_id']
        else:
            org_id = None
        ...
        for index, reagent_template in enumerate(exp_template.reagent_templates.all().order_by('description')): #type: ignore
            ...
            for reagent_material_template in reagent_template.reagent_material_template_rt.all().order_by('description'): #type: ignore
                ...
                for reagent_material_value_template in reagent_material_template.reagent_material_value_template_rmt.filter(description='concentration'): #type: ignore
                    ...
                    initial_data.append({'reagent_template_uuid': reagent_material_template.uuid, 
                                'material_type':material_type.uuid,
                                'desired_concentration': reagent_material_value_template.default_value.nominal_value}) #type: ignore
            ...
            if mat_types_list:
                fset = self.ReagentFormSet(prefix=f'reagent_{index}', 
                                           ...
                                           'reagent_index':index}) #type: ignore
        ...
        # Dead volume form
        initial: dict[str, Val] = {'value':Val.from_dict({'value':4000, 'unit': 'uL', 'type':'num'})}
    ...
    def get_action_parameter_forms(self, exp_uuid: str, context: dict[str, Any] , template: bool=True):
        initial_q1: list[Any]
        q1_details: list[Any]
    ...
    def process_formsets(self, request: HttpRequest, context: dict[str, Any]) -> dict[str, Any]:
        ...
        if all([exp_name_form.is_valid(),
            ...
            exp_name = exp_name_form.cleaned_data['exp_name']
            # make the experiment copy: this will be our new experiment
            experiment_copy_uuid: str = experiment_copy(str(exp_template.uuid), exp_name)
