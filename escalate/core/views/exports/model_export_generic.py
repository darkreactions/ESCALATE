...
from core.utils_no_dependencies.view_utils import (
...
class GenericModelExport(GenericModelListBase, View):
    ...
    def export_csv(self):
        ...
        if (
            queryset_serialized := self.request.session.get(
                f"{model_name}_queryset_serialized", None
            )
        ) != None:
            model_objects = [
                serialized_obj.object
                for serialized_obj in serializers.deserialize(
                    "json", queryset_serialized
                )
            ]
        else:
        ...
        for obj in model_objects:
            ...
            for col_name in self.column_names:
                ...
                for field in necessary_fields:
                    ...
                    if (
                        field.split(".") == 1
                        and self.model._meta.get_field(field).__class__.__name__
                        == "ManyToManyField"
                    ):
