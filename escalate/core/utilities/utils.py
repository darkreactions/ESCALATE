...
def experiment_copy(template_experiment_uuid, copy_experiment_description):    
    ...
    old_bom_get = BillOfMaterials.objects.filter(experiment=template_experiment_uuid)
    ...
    experiment_workflow_filter = ExperimentWorkflow.objects.all().filter(experiment=old_exp_get)
