...
def generate_action_def_json(action_defs, exp_template_uuid):
    ...
    for vt in VesselTemplate.objects.all():  # include all vessels as destinations
        source_choices.append(vt.description)
        dest_choices.append(
            vt.description
        )  ##TODO: add wells for decomposable vessels (needed for str lookup to save action template)
                    ...
                    if "wells" in a.destination:  # individual well-level actions
                        ...
                        for well_desc, well_vessel in plate_wells.items():
                            ...
                            if source_bbm:
                                ...
                                description = f"{action.description} : {destination_bbm.description}"
