...
def api_root(request, format=None):
    ...
    for view_name in sorted(rest_exposed_url_views):
        ...
        response_object[name] = reverse(name + "-list", request=request, format=format)
...
registered = router.register(
    "experiment-template",
    viewsets.ExperimentTemplateViewSet,
    basename="experimenttemplate",
)
name = "experimenttemplate"
registered.register(
    "notes",
    viewsets.NoteViewSet,
    basename=f"{name}-note",
    parents_query_lookups=["ref_note_uuid"],
)
registered.register(
    "tags",
    viewsets.TagAssignViewSet,
    basename=f"{name}-tag",
    parents_query_lookups=["ref_tag"],
)
registered.register(
    "edocs",
    viewsets.EdocumentViewSet,
    basename=f"{name}-edoc",
    parents_query_lookups=["ref_edocument_uuid"],
)
registered.register(
    "create",
    viewsets.ExperimentCreateViewSet,
    basename=f"{name}-create",
...
for view in rest_nested_url_views:
    ...
    many_to_one_rel_model = [
        model._meta.get_field(url).remote_field.model.__name__
        for url in many_to_one_names
    ]
    ...
    registered = router.register(
        rf"{dasherized_name}", getattr(viewsets, view + "ViewSet"), basename=name
    )
    ...
    for i, url in enumerate(url_names):
        ...
        registered.register(
            rf"{url}",
            getattr(viewsets, related_model_name + "ViewSet"),
            basename=f"{name}-{url}",
            parents_query_lookups=[related_names[i]],
        )
    ...
    if name == "material" or name == "mixture":
        registered.register(
            "property",
            viewsets.PropertyViewSet,
            basename=f"{name}-property",
            parents_query_lookups=["material"],
        )
    if name == "action":
        registered.register(
            "parameter",
            viewsets.ParameterViewSet,
            basename=f"{name}-parameter",
            parents_query_lookups=["action"],
        )
