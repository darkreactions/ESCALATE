...
actor = get_data(
    "actor",
    {"organization__isnull": True, "description__contains": "Shekar"},
...
print(plate_96)
# %%
well_loc_prop_def = get_data(
    "propertydef", {"short_description": "well_loc"}, headers=token_header
)
well_vol_prop_def = get_data(
    "propertydef", {"short_description": "well_vol"}, headers=token_header
)
well_ord_prop_def = get_data(
    "propertydef", {"short_description": "well_ord"}, headers=token_header
)
# %%
...
for ord, well in enumerate(well_names):
    mat_data = {
        "description": f"96 well plate: {well[1]}{well[0]}",
        "consumable": False,
        "actor": actor["url"],
        "status": test_status["url"],
    }
    ...
    well_entry = get_data(
        "material", data={"description": mat_data["description"]}, headers=token_header
    )
    mat_comp = {
        "composite": plate_96["url"],
    ...
    well_comp_entry = get_data(
        "mixture",
        data={"component_description": well_entry["description"]},
        headers=token_header,
    )
    mat_prop = {
        "material": well_entry["url"],
...
# %%
r_materials = {}
r_materials["organic"] = get_data(
    "material",
    {"description": "4-Hydroxyphenethylammonium iodide"},
    headers=token_header,
)
r_materials["inorganic"] = get_data(
    "material", {"description": "Lead Diiodide"}, headers=token_header
)
r_materials["solvent"] = get_data(
    "material", {"description": "Dimethylformamide"}, headers=token_header
)
r_materials["acid"] = get_data(
    "material", {"description": "Formic Acid"}, headers=token_header
)
...
material_composites = [
    ...
    [
        r_materials["Reagent2"]["url"],
        r_materials["inorganic"]["url"],
        False,
        "template",
    ],
