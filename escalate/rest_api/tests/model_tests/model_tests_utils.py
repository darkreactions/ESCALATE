...
import datetime
import json
from core.models import SystemtoolType, Organization, Systemtool
...
status_codes = {POST: 201, PUT: 200, DELETE: 204, GET: 200, ERROR: 404}
...
def random_model_dict(model, **kwargs):
    ...
    can_generate_random_val = lambda field: not (
        field.__class__.__name__ == "RetUUIDField"
        or field.__class__.__name__ == "SlugField"
        or field.__class__.__name__ == "UUIDField"
        or field.name == "add_date"
        or field.name == "mod_date"
        or field.name == "full_description"
        or field.__class__.__name__ == "ManyToManyField"
        or field.__class__.__name__ == "ForeignKey"
        or field.__class__.__name__ == "OneToOneField"
        or field.editable == False
    )
    ...
    for field_name, field_obj in dict_fields.items():
        ...
        if field_obj.choices != None:
            ...
            if field_class_name == "CharField":
                ...
                field_class_name == "ArrayField"
                and field_obj.base_field.__class__.__name__ == "CharField"
            ):
                ...
                rand_year = random.randint(
                    2000, int(str(datetime.datetime.today().year))
                )
                ...
                model_dict[field_name] = str(datetime.datetime.today()).replace(
                    " ", "T"
                )
            elif (
                field_class_name == "FloatField"
                or field_class_name == "BigIntegerField"
                or field_class_name == "IntegerField"
            ):
                ...
                rand_unit = "".join(
                    random.choices(string.ascii_lowercase + string.ascii_uppercase, k=3)
                )
                if rand_type == "text":
                    rand_value = "".join(
                        random.choices(
                            string.ascii_lowercase + string.ascii_uppercase, k=10
                        )
                    )
                elif rand_type == "num":
                    ...
                    rand_value = [
                        (True if random.uniform(0, 1) > 0.5 else False)
                        for i in range(10)
                    ]
                else:
                    rand_value = None
    ...
    for field_name, value in kwargs.items():
        assert (
            field_name in _field_names
        ), f"Invalid field name: {field_name} in {model._meta.model_name}. Valid field names are: {_field_names}"
        valid_field = [f for f in all_fields if f.name == field_name]
        assert (
            valid_field[0].editable != False
        ), f"The {field_name} {valid_field[0].__class__.__name__} in the {model.__name__} model test is not editable. Remove it from the test case."
        ...
        [
            ...
            },
            {
                ...
                "body": random_model_dict(
                    Systemtool,
                    vendor_organization=f"org{number}__url",
                    systemtool_type=f"systemtooltype{number}__url",
                ),
                "args": [],
                "query_params": [],
                "is_valid_response": {
