...
r_material_property = post_data('materialproperty', material_property_data, token_header)
...
for action_def in action_def_data:
    ...
    r = requests.post(f'{base_url}/actiondef/',
                    data=json.dumps(action_def),
                    headers=token_header).json()
...
parameter_def_data = [
    ['duration_qualitative', {'value': "briefly", 'unit': "", 'type': "text"}, actor['url'], dev_test_status['url']],
    ['intensity_qualitative', {'value': "gently", 'unit': "", 'type': "text"}, actor['url'], dev_test_status['url']],
    ['volume_floz', {'value': 0, 'unit': "floz", 'type': "num"}, actor['url'], dev_test_status['url']],
    ['count', {'value': 0, 'unit': "count", 'type': "int"}, actor['url'], dev_test_status['url']],
    ['amount_qualitative', {'value': "all", 'unit': "", 'type': "text"}, actor['url'], dev_test_status['url']]
]
r_parameter_def = {}
for param_def in parameter_def_data:
    param_def = dict(zip(parameter_def_fields, param_def))
    r = requests.post(f'{base_url}/parameterdef/',
                      data=json.dumps(param_def),
                      headers=token_header).json()
